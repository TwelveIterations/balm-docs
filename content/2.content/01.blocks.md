...
You can obtain a `BalmBlockRegistrar` through the `BalmRegistrars` instance passed to your mod initializer, or by registering your mod as a `BalmModule`.
...
Most of the time you don't actually need to store or access a block instance directly. Instead, Balm provides a `DeferredBlock` interface with:
...
- `asResourceKey()`, which returns the block's registry key
- `asBlockItemId()`, which returns the block-item reference

This should cover most uses of what would otherwise be a `Block` instance. `BlockLike` also provides `asBlock()` for APIs that require the resolved block.

:::note
Do not try to store `Block` instances in your ModBlocks class by using `asDeferredBlock().asBlock()` in your registration code, as that is too early to access the resolved block on NeoForge and Forge.
