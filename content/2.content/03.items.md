...
You can obtain a `BalmItemRegistrar` through the `BalmRegistrars` instance passed to your mod initializer, or by registering your mod as a `BalmModule`.
...
Most of the time you don't actually need to store or access an item instance directly. Instead, Balm provides a `DeferredItem` interface with:
...
- `asHolder()`, which returns the registered item as a `Holder<Item>`
- `asResourceKey()`, which returns the item's registry key
