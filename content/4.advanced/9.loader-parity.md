...
| `BalmCapabilities#registerType` | ✅ | ✅ | ⛔ | `registerType` will make a type known to Balm, but registration on Forge is only possible with a Forge-specific annotation, and therefore no longer trivially achievable in common code. |
| `BalmRegistrar#createCustomRegistry` | ✅ | ✅ | ⛔ | Balm does not support code-driven custom registries on Forge. |
...
| `BalmLootTables` | ⚠️ | ⚠️ | ⚠️ | Balm does not support ordering of individual loot modifiers at the moment - all Balm-registered modifiers are treated as one batch. |
| `.asHolder()` | ✅ | ✅ | ⚠️ | Must only be called after deferred registration has finished on Forge. |
